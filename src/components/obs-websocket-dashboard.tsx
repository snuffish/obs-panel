...
export function ObsWebsocketDashboard() {
  ...
  useEffect(() => {
    ...
    const newCpuUsage = Math.random() * 100
      ...
      setCpuHistory(prev => [...prev.slice(-19), { time: new Date().toLocaleTimeString(), usage: newCpuUsage }])
  ...
  return (
    <div className="p-4 space-y-4">
      <h1 className="text-2xl font-bold text-white">OBS WebSocket Connection Dashboard</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Video className={streamStatus === "Live" ? "text-red-500" : "text-neutral-500 dark:text-neutral-400"} />
          </CardHeader>
          <CardContent>
            <Badge variant={streamStatus === "Live" ? "destructive" : "secondary"}>{streamStatus}</Badge>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {streamInfo.services.map((service, index) => (
                <div key={index} className="flex items-center space-x-4 p-2 border border-neutral-200 rounded-md dark:border-neutral-800">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {activeSources.map((source) => (
              <div key={source.id} className="flex items-center space-x-4 p-2 border border-neutral-200 rounded-md dark:border-neutral-800">
                {source.type === "video" ? (
                ...
                <div className="flex-1">
                  ...
                  <div className="text-sm text-neutral-500 dark:text-neutral-400">{source.type}</div>
                </div>
